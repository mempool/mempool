    ...
    this.subscription = this.route.paramMap
      .pipe(
        switchMap((params: ParamMap) => {
          ...
          this.seoService.setDescription($localize`:@@meta.description.bells.transaction:Get real-time status, addresses, fees, script info, and more for ${this.stateService.network==='liquid'||this.stateService.network==='liquidtestnet'?'Liquid':'Bells'}${seoDescriptionNetwork(this.stateService.network)} transaction with txid {txid}.`);
