...
import { fillTapTree } from '@app/shared/transaction.utils';
...
export class AddressComponent implements OnInit, OnDestroy {
  ...
  submitPsbt(): void {
    ...
    try {
      fillTapTree(this.addressTypeInfo, sanitized);
      this.addressTypeInfo = this.addressTypeInfo.clone();
      this.hasTapTree = this.addressTypeInfo.tapscript && this.addressTypeInfo.scripts.values().next().value.taprootInfo.scriptPath.merkleBranches.length > 0;
      this.tapTreeIncomplete = false;
      this.taprootPsbtExpanded = false;
      ...
      if (error instanceof Error) {
        ...
        this.psbtError = 'Unable to parse PSBT';
      }
    }
  }

  isTapTreeIncomplete(tapTreeIncomplete: boolean): void {
    this.tapTreeIncomplete = tapTreeIncomplete;
  }

  showTaprootPsbtButton(): boolean {
    const isBitcoin = this.stateService.network !== 'liquid' && this.stateService.network !== 'liquidtestnet';
    // Show the button if the taproot address has no script spend, or an incomplete tap tree
    return this.addressTypeInfo.type === 'v1_p2tr' && isBitcoin && (!this.addressTypeInfo.tapscript || (this.hasTapTree && this.tapTreeIncomplete));
  }
