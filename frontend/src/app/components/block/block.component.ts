...
export class BlockComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.txsLoadingStatus$ = this.route.paramMap.pipe(
      ...
      map(indicators =>
        indicators['blocktxs-' + this.blockHash] !== undefined ? indicators['blocktxs-' + this.blockHash] : 0
      )
    );
      ...
      .subscribe(
        (transactions: Transaction[]) => {
          if (this.fees === undefined && transactions[0]) {
            this.fees =
              transactions[0].vout.reduce((acc: number, curr: Vout) => acc + curr.value, 0) / 100000000 -
              this.blockSubsidy;
          }
  ...
  pageChange(page: number) {
    ...
    this.electrsApiService.getBlockTransactions$(this.block.id, start).subscribe(transactions => {
      this.transactions = transactions;
      this.isLoadingTransactions = false;
    });
  }
