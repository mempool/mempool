...
export class BisqMainDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.usdPrice$ = this.stateService.conversions$.asObservable().pipe(map(conversions => conversions.USD));

    this.volumes$ = this.bisqApiService.getAllVolumesDay$().pipe(
      map(volumes => {
    ...
    this.tickers$ = combineLatest([
      ...
      map(([tickers, markets, volumes]) => {
        ...
        for (const t in tickers) {
          ...
          mappedTicker.name = `${
            mappedTicker.market.rtype === 'crypto' ? mappedTicker.market.lname : mappedTicker.market.rname
          } (${mappedTicker.market.rtype === 'crypto' ? mappedTicker.market.lsymbol : mappedTicker.market.rsymbol}`;
    ...
    this.trades$ = combineLatest([this.bisqApiService.getMarketTrades$('all'), getMarkets]).pipe(
      map(([trades, markets]) => {
        if (!this.stateService.env.OFFICIAL_BISQ_MARKETS) {
    ...
    this.hlocData$ = this.bisqApiService.getMarketsHloc$('btc_usd', 'day').pipe(
      map(hlocData => {
