...
export class AddressComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.addressLoadingStatus$ = this.route.paramMap.pipe(
      ...
      map(indicators =>
        indicators['address-' + this.addressString] !== undefined ? indicators['address-' + this.addressString] : 0
      )
    );
      ...
      .pipe(
        switchMap((params: ParamMap) => {
          ...
          return merge(
            ...
            switchMap(() =>
              this.electrsApiService.getAddress$(this.addressString).pipe(
                catchError(err => {
  ...
  loadMore() {
    ...
    this.electrsApiService.getAddressTransactionsFromHash$(this.address.address, this.lastTransactionTxId).subscribe(
      (transactions: Transaction[]) => {
        this.lastTransactionTxId = transactions[transactions.length - 1].txid;
