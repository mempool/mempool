/**
 * Mempool Server Configuration Types
 * Version: 5.0.0
...
class Config implements IConfig {
  ...
  // A simple merge function that shallow-merges top-level properties.
  // For deeper merge behavior, consider using lodash.merge.
  merge = (...objects: object[]): IConfig => {
    return objects.reduce((prev, next) => {
      Object.keys(prev).forEach(key => {
        // Merge each section so that defaults override configFromFile if not set
        next[key] = { ...prev[key], ...next[key] };
      });
      return next as IConfig;
    });
  }
}
