...
Cypress.Commands.add('waitForSkeletonGone', (timeout: number = 15000) => {
  cy.waitUntil(() => {
    return Cypress.$('.skeleton-loader').length === 0;
  }, { verbose: true, description: "waitForSkeletonGone", errorMsg: "skeleton loaders never went away", timeout: timeout, interval: 1000 });
});
