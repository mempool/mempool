...
class BisqMarketsApi {
    constructor() {
        ...
        this.activeCryptoCurrencyData = [];
        this.activeFiatCurrencyData = [];
        this.offersData = [];
        this.tradesData = [];
    ...
    setCurrencyData(cryptoCurrency, fiatCurrency, activeCryptoCurrency, activeFiatCurrency) {
    ...
    getTrades(market, timestamp_from, timestamp_to, trade_id_from, trade_id_to, direction, limit = 100, sort = 'desc') {
        ...
        const matches = this.getTradesByCriteria(_market, timestamp_to, timestamp_from, trade_id_to, trade_id_from, direction, sort, limit, false);
        if (sort === 'asc') {
    ...
    getVolumes(market, timestamp_from, timestamp_to, interval = 'auto', milliseconds, timestamp = 'yes') {
        if (milliseconds) {
        ...
        const trades = this.getTradesByCriteria(market, timestamp_to, timestamp_from, undefined, undefined, undefined, 'asc', Number.MAX_SAFE_INTEGER);
        if (interval === 'auto') {
    ...
    getTickerFromMarket(market) {
        let ticker;
        const timestamp_from = datetime.strtotime('-24 hour');
        const timestamp_to = new Date().getTime() / 1000;
        const trades = this.getTradesByCriteria(market, timestamp_to, timestamp_from, undefined, undefined, undefined, 'asc', Number.MAX_SAFE_INTEGER);
        const periods = Object.values(this.getTradesSummarized(trades, timestamp_from));
        const allCurrencies = this.getCurrencies();
    ...
    getHloc(market, interval = 'auto', timestamp_from, timestamp_to, milliseconds, timestamp = 'yes') {
        if (milliseconds) {
            timestamp_from = timestamp_from ? timestamp_from / 1000 : timestamp_from;
            timestamp_to = timestamp_to ? timestamp_to / 1000 : timestamp_to;
        }
    ...
    getTradesSummarized(trades, timestamp_from, interval) {
        const intervals = {};
        const intervals_prices = {};
        for (const trade of trades) {
            const traded_at = trade.tradeDate / 1000;
            ...
            if (price) {
                ...
                period['avg'] = intervals_prices[interval_start]['rightvol'].reduce((p, c) => c + p, 0)
                    / intervals_prices[interval_start]['leftvol'].reduce((c, p) => c + p, 0) * 100000000;
    ...
    getTradesByCriteria(market, timestamp_to, timestamp_from, trade_id_to, trade_id_from, direction, sort, limit, integerAmounts = true) {
        let trade_id_from_ts = null;
        let trade_id_to_ts = null;
        const allCurrencies = this.getCurrencies();
        const timestampFromMilli = timestamp_from * 1000;
        const timestampToMilli = timestamp_to * 1000;
...
exports.default = new BisqMarketsApi();
