...
const svgFactory = {
  bitcoin: (width, height, color) => {
    return `<svg xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" viewBox="0 0 64 64" width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:cc="http://creativecommons.org/ns#" xmlns:dc="http://purl.org/dc/elements/1.1/">
...
const literalImages = [
  {
    ...
    svg: getNetworkLogo('bitcoin', 64),
  },
  {
    key: 'bitcoin-signet-logo',
    svg: getNetworkLogo('signet', 64),
  },
  {
    key: 'bitcoin-testnet-logo',
    svg: getNetworkLogo('testnet', 64),
  },
  {
...
async function preloadImages(): Promise<void> {
  ...
  for (const image of literalImages) {
    const logo = new Image();
    logo.src = `data:image/svg+xml;base64,${Buffer.from(image.svg).toString('base64')}`;
    imageMap.set(image.key, logo);
  }
}
