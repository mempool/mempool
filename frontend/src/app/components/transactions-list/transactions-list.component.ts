...
export class TransactionsListComponent implements OnInit, OnChanges {
  ...
  toggleLoadAll(txObj: any) {
    let index = this.transactions.indexOf(txObj);
    this.transactions[index]["@loadingInputsStatus"] = true;
    this.transactions[index]["@loadAll"]=! this.transactions[index]["@loadAll"];
    if(Object.keys(this.transactions[index]['@expTx']).length === 0)
    {
      this.electrsApiService.getTransaction$(txObj.txid).subscribe((tx) => {
        this.transactions[index]["@expTx"] = tx;
        this.transactions[index]["@loadingInputsStatus"] = false;
      })
    }
    else
    {
