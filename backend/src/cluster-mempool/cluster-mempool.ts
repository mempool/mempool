...
export class ClusterMempool {
  ...
  private adjustedSize(tx: MempoolTransactionExtended): number {
  ...
  private createClusterFromTxids(
    ...
    for (const txid of txids) {
      ...
      const clusterTx = depgraph.addTransaction(txid, this.effectiveFee(txid, tx), this.adjustedSize(tx), tx.order ?? 0);
  ...
  private addToExistingCluster(
    ...
    const clusterTx = cluster.depgraph.addTransaction(txid, this.effectiveFee(txid, tx), this.adjustedSize(tx), tx.order ?? 0);
  ...
  private mergeAndAddToCluster(
    ...
    const clusterTx = primary.depgraph.addTransaction(txid, this.effectiveFee(txid, tx), this.adjustedSize(tx), tx.order ?? 0);
