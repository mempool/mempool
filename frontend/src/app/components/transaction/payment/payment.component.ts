...
export class PaymentComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    .pipe(
      ...
      if (this.rbfInfo) {
        ...
        // the tree is rooted at the newest version, so anything else means
        // this payment has been superseded, even if we never saw the event
        if (!this.tx?.status?.confirmed && this.latestReplacement && this.latestReplacement !== this.txId) {
          this.replaced = true;
    ...
    .pipe(
      ...
      if (!tx) {
        ...
        // Nothing to show a payment for: not in the mempool, not mined, not
        // an RBF replacement. The full tx page waits for it to appear.
        if (!this.replaced) {
          this.viewFullDetails(false);
        }
        return;
      }
    ...
    this.subscription = this.zoneService.wrapObservable(this.route.paramMap
      ...
      .subscribe((tx: Transaction) => {
        ...
        this.fetchRbfHistory$.next(tx.txid);
        this.setAmount();
        this.updateConfirmations();
        this.txChanged$.next(true);
        this.markBlock();
  ...
  private destinationIsAddress(): boolean {
    ...
    return !!this.destination && getRegex('address', network).test(this.destination);
  }
  ...
  viewFullDetails(showAlert = true): void {
    const baseRoute = [this.relativeUrlPipe.transform('/tx'), this.txId];
    // only flag the destination when it is an address this transaction really
    // does not pay, which is the one thing the alert on the tx page claims
    if (showAlert && this.tx && this.destinationIsAddress() && !this.destinationInOutputs(this.tx)) {
  ...
  // we only have this transaction from the rbf cache, so it has left the
  // mempool without a replacement to point at and without confirming
  get removed(): boolean {
    return !!this.tx && this.isCached && this.rbfChecked && !this.replaced && !this.tx.status?.confirmed;
  }
